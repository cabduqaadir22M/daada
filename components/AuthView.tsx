...
export const AuthView: React.FC<AuthViewProps> = ({ onAuthSuccess }) => {
  ...
  const handleStart = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      // Check if user already exists in local DB to resume history
    ...
    <div className="min-h-screen flex items-center justify-center bg-white dark:bg-black p-6 font-sans">
      <div className="w-full max-w-[420px] animate-in fade-in zoom-in duration-1000">
        <div className="flex flex-col items-center mb-12 text-center">
          ...
          <h1 className="text-4xl font-black tracking-tighter text-zinc-900 dark:text-white">Aqoonsi</h1>
          <p className="text-zinc-500 text-[10px] mt-3 font-black uppercase tracking-[0.4em] opacity-50">Enter your name to begin intelligence</p>
        </div>
