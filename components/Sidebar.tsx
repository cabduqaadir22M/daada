...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 w-[280px] md:w-64 bg-zinc-50 dark:bg-[#080808] border-none flex flex-col z-50 transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-5 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Logo className="w-7 h-7" hideText={true} />
              <span className="font-extrabold text-xl tracking-tight text-zinc-900 dark:text-white">Daadir.ai</span>
            </div>
            <button onClick={onToggleTheme} className="w-9 h-9 flex items-center justify-center rounded-xl hover:bg-zinc-200 dark:hover:bg-zinc-900 transition-colors text-zinc-500 border-none">
          ...
          >
            <span className="text-[11px] font-bold tracking-tight">Start new chat</span>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-3 space-y-1 border-none">
          <div className="px-4 mb-2">
            <h3 className="text-[9px] font-bold text-zinc-400 tracking-wider opacity-60">Recent history</h3>
          </div>
          {sessions.length === 0 ? (
            <div className="px-6 py-10 text-center opacity-30">
              <p className="text-[10px] font-bold italic tracking-wider">No history yet</p>
            </div>
          ) : (
            sessions.map(s => (
              <button
                ...
                className={`w-full text-left px-4 py-3.5 rounded-2xl text-[12px] font-bold truncate transition-all ${activeSessionId === s.id ? 'bg-white dark:bg-zinc-900 text-blue-600 dark:text-blue-400 shadow-md scale-[1.02]' : 'text-zinc-500 hover:bg-zinc-200 dark:hover:bg-zinc-900/50 hover:text-zinc-900'}`}
              >
        ...
        <div className="p-5 border-none space-y-3">
           ...
           >
             ...
             <span className="text-[11px] font-bold">Image generation</span>
           </button>

           <div className="flex items-center gap-3 px-4 py-4 rounded-2xl bg-white dark:bg-zinc-900 shadow-lg border-none">
              <div className="w-9 h-9 rounded-xl bg-zinc-900 dark:bg-zinc-100 flex items-center justify-center text-xs text-white dark:text-black font-black">
              ...
              <div className="flex-1 min-w-0">
                ...
                <button onClick={onLogOut} className="text-[9px] font-bold tracking-tight text-red-500 hover:opacity-70 transition-opacity">
                  Log out
                </button>
              </div>
           </div>
