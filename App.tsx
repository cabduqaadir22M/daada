  ...
  const handleSend = async () => {
    ...
    if (!sId || !session) {
      ...
      session = {
        id: sId,
        userId: user.id,
        title: trimmedInput.slice(0, 30) || 'New Conversation',
        messages: [],
        createdAt: Date.now(),
        updatedAt: Date.now()
      };
    ...
    const userMsg: Message = {
      ...
      role: 'user',
      content: trimmedInput || "Synapse signal initiated",
      timestamp: Date.now(),
    ...
    try {
      ...
      const finalSession = { 
        ...session, 
        messages: [...session.messages, userMsg, { ...assistantPlaceholder, content: fullContent }],
        updatedAt: Date.now()
      };
      ...
      setSessions(prev => prev.map(s => {
        ...
        return {
          ...s,
          messages: s.messages.map(m => m.id === assistantId ? { ...m, content: "Neural Link Synchronization Failed. Aqli is reconnecting. Please resend your last prompt." } : m)
        };
      }));
    } finally {
      setIsLoading(false);
      ...
      <main className="flex-1 flex flex-col relative w-full h-full min-w-0 border-none">
        <header className="h-16 md:h-20 flex items-center justify-between px-6 md:px-10 bg-white/80 dark:bg-black/80 backdrop-blur-xl z-10 shrink-0 border-none">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-[11px] font-black text-blue-500 uppercase tracking-[0.3em]">Neural Processor</span>
              <span className="text-[9px] font-bold text-zinc-400 dark:text-zinc-600 uppercase tracking-widest">Aqli v5.0 Deep-Link</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
             {view !== 'chat' && (
               <button onClick={() => setView('chat')} className="text-[10px] font-black text-zinc-500 hover:text-black dark:hover:text-white transition-all bg-zinc-100 dark:bg-zinc-900 px-5 py-2 rounded-full uppercase tracking-widest border-none">
                 CONVERSATIONS
               </button>
             )}
        ...
        <div className="flex-1 w-full overflow-hidden flex flex-col relative border-none">
          {view === 'image-gen' ? (
            ...
            <>
              <div ref={scrollRef} className="flex-1 overflow-y-auto pt-6 pb-40 md:pb-52 w-full custom-scrollbar border-none">
                  ...
                  {!activeSession || activeSession.messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center mt-24 md:mt-40 text-center animate-in fade-in duration-1000 w-full border-none">
                      ...
                      <div className="flex gap-4 opacity-30 border-none">
                         <span className="text-[9px] font-black uppercase tracking-[0.4em] text-zinc-500">Intelligent</span>
                         <span className="text-[9px] font-black uppercase tracking-[0.4em] text-zinc-500">Persistent</span>
                         <span className="text-[9px] font-black uppercase tracking-[0.4em] text-zinc-500">Creative</span>
                      </div>
                    </div>
                  ) : (
